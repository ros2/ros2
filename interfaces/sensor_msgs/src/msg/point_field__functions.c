...
// sensor_msgs/msg/PointField functions implementation
...
sensor_msgs__msg__PointField * sensor_msgs__msg__PointField__create(void)
{
  ...
  sensor_msgs__msg__PointField * msg =
    (sensor_msgs__msg__PointField *)allocator.allocate(
      sizeof(sensor_msgs__msg__PointField), allocator.state);
  if (!msg) { return NULL; }
  if (!sensor_msgs__msg__PointField__init(msg)) {
  ...
  sensor_msgs__msg__PointField__Sequence * array, size_t size)
{
  ...
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (size) {
    array->data = (sensor_msgs__msg__PointField *)allocator.zero_allocate(
      size, sizeof(sensor_msgs__msg__PointField), allocator.state);
    if (!array->data) { return false; }
    for (size_t i = 0; i < size; ++i) {
      if (!sensor_msgs__msg__PointField__init(&array->data[i])) {
        for (size_t j = 0; j < i; ++j) {
          sensor_msgs__msg__PointField__fini(&array->data[j]);
        }
