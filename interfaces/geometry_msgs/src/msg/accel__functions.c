...
// geometry_msgs/msg/Accel functions implementation
...
geometry_msgs__msg__Accel * geometry_msgs__msg__Accel__create(void)
{
  ...
  geometry_msgs__msg__Accel * msg =
    (geometry_msgs__msg__Accel *)allocator.allocate(
      sizeof(geometry_msgs__msg__Accel), allocator.state);
  if (!msg) { return NULL; }
  if (!geometry_msgs__msg__Accel__init(msg)) {
  ...
  geometry_msgs__msg__Accel__Sequence * array, size_t size)
{
  ...
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (size) {
    array->data = (geometry_msgs__msg__Accel *)allocator.zero_allocate(
      size, sizeof(geometry_msgs__msg__Accel), allocator.state);
    if (!array->data) { return false; }
    for (size_t i = 0; i < size; ++i) {
      if (!geometry_msgs__msg__Accel__init(&array->data[i])) {
        for (size_t j = 0; j < i; ++j) {
          geometry_msgs__msg__Accel__fini(&array->data[j]);
        }
