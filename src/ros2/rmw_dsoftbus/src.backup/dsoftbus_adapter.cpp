...
// Debug signal handler for crash investigation
static void debug_signal_handler(int sig) {
    fprintf(stderr, "\n[RMW_DSOFTBUS SIGNAL HANDLER] Caught signal %d\n", sig);
    fflush(stderr);
...
static void install_debug_signal_handler() {
    static bool installed = false;
    if (!installed) {
        signal(SIGSEGV, debug_signal_handler);
        signal(SIGBUS, debug_signal_handler);
        signal(SIGABRT, debug_signal_handler);
        fprintf(stderr, "[DEBUG] Installed signal handler for SIGSEGV/SIGBUS/SIGABRT\n");
        fflush(stderr);
        installed = true;
    }
}
...
#define SET_FTOKEN_ID 4
#define ACCESS_TOKENID_GET_TOKENID _IOR(ACCESS_TOKEN_ID_IOCTL_BASE, GET_TOKEN_ID, uint64_t)
#define ACCESS_TOKENID_SET_TOKENID _IOW(ACCESS_TOKEN_ID_IOCTL_BASE, SET_TOKEN_ID, uint64_t)
#define ACCESS_TOKENID_SET_FTOKENID _IOW(ACCESS_TOKEN_ID_IOCTL_BASE, SET_FTOKEN_ID, uint64_t)
...
static bool init_native_token() {
    ...
    const char* token_id_env = std::getenv("RMW_DSOFTBUS_TOKEN_ID");
    if (token_id_env && *token_id_env) {
        char* end = nullptr;
        unsigned long long parsed = std::strtoull(token_id_env, &end, 0);
        if (end && *end == '\0' && parsed != 0ULL) {
            tokenId = static_cast<uint64_t>(parsed);
        } else {
            RCUTILS_LOG_WARN_NAMED("rmw_dsoftbus",
                "Invalid RMW_DSOFTBUS_TOKEN_ID='%s', using default tokenId=%lu",
                token_id_env, (unsigned long)tokenId);
        }
    }
    ...
    // Best-effort: also set first caller token ID so chained IPC keeps the same identity.
    (void)ioctl(fd, ACCESS_TOKENID_SET_FTOKENID, &tokenId);

    uint64_t verify = 0;
    if (ioctl(fd, ACCESS_TOKENID_GET_TOKENID, &verify) == 0) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "NativeToken set: tokenId=%lu (verify=%lu)",
            (unsigned long)tokenId, (unsigned long)verify);
    } else {
        RCUTILS_LOG_WARN_NAMED("rmw_dsoftbus", "NativeToken set: tokenId=%lu (verify read failed errno=%d)",
            (unsigned long)tokenId, errno);
    }
...
typedef int32_t (*OpenKhSessionFunc)(const char*, const char*, char*, char*, SessionType, KhTransParam*);
...
static void* g_softbus_kh_handle = nullptr;  // Prefer server-side KH_TRANS symbols when available

static const char* g_loaded_lib_path = nullptr;
static const char* g_loaded_kh_lib_path = nullptr;
...
static bool load_softbus_symbols() {
    ...
    // Prefer KH_TRANS symbols from the server library when available.
    // On some KaihongOS builds, the client library provides KH_TRANS entrypoints but still depends on IPC context,
    // which can break cross-device usage for native ELF processes.
    const char* kh_lib_paths[] = {
    ...
    for (int i = 0; !g_softbus_kh_handle && kh_lib_paths[i] != nullptr; ++i) {
        g_softbus_kh_handle = dlopen(kh_lib_paths[i], RTLD_NOW | RTLD_GLOBAL);
        if (g_softbus_kh_handle) {
            g_loaded_kh_lib_path = kh_lib_paths[i];
        }
    }

    if (g_softbus_kh_handle) {
        auto kh_send = (KhSendBytesFunc)dlsym(g_softbus_kh_handle, "KhSendBytes");
        auto kh_create = (CreateKhSessionServerFunc)dlsym(g_softbus_kh_handle, "CreateKhSessionServer");
        auto kh_remove = (RemoveKhSessionServerFunc)dlsym(g_softbus_kh_handle, "RemoveKhSessionServer");
        auto kh_open = (OpenKhSessionFunc)dlsym(g_softbus_kh_handle, "OpenKhSession");
        auto kh_close = (CloseKhSessionFunc)dlsym(g_softbus_kh_handle, "CloseKhSession");

        if (kh_send && kh_create && kh_open) {
            g_KhSendBytes = kh_send;
            g_CreateKhSessionServer = kh_create;
            g_RemoveKhSessionServer = kh_remove;
            g_OpenKhSession = kh_open;
            g_CloseKhSession = kh_close;
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Using KH_TRANS symbols from: %s", g_loaded_kh_lib_path);
        } else {
            RCUTILS_LOG_WARN_NAMED("rmw_dsoftbus", "KH_TRANS symbols incomplete in: %s (keeping client symbols)",
                    g_loaded_kh_lib_path ? g_loaded_kh_lib_path : "<unknown>");
        }
    }
...
// MUST be outside #ifdef block as it's used in both REAL_SOFTBUS and STUB modes
...
struct SessionInfo {
    ...
    enum class Transport : uint8_t { STANDARD = 0, KH_TRANS = 1 };
    Transport transport{Transport::STANDARD};
    uint32_t send_index{0};  // KH_TRANS message index (must increment for each send)
};

struct SessionServerInfo {
    int server_id;
    SessionInfo::Transport transport{SessionInfo::Transport::STANDARD};
};
...
struct DsoftbusState {
    ...
    std::unordered_map<std::string, SessionServerInfo> session_servers;
    ...
    // Open callbacks can be delivered before the caller registers a PendingSessionOpen (e.g., synchronous OpenKhSession).
    // Stash early failures so callers can observe them without timing out.
    std::unordered_map<int, int> early_open_results;
...
#ifdef RMW_DSOFTBUS_USE_REAL_SOFTBUS
// ============================================================================
// Bus Center Initialization (RegNodeDeviceStateCb) - required for discovery/LNN
// ============================================================================

static bool g_bus_center_registered = false;

static void on_node_online(NodeBasicInfo* info) {
    fprintf(stderr, "[CALLBACK] on_node_online ENTER: info=%p\n", (void*)info);
    fflush(stderr);
    if (!info) return;
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "BusCenter: node online: name=%s, networkId=%.32s...",
            info->deviceName, info->networkId);
}

static void on_node_offline(NodeBasicInfo* info) {
    fprintf(stderr, "[CALLBACK] on_node_offline ENTER: info=%p\n", (void*)info);
    fflush(stderr);
    if (!info) return;
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "BusCenter: node offline: name=%s, networkId=%.32s...",
            info->deviceName, info->networkId);
}

static void on_node_basic_info_changed(NodeBasicInfoType type, NodeBasicInfo* info) {
    fprintf(stderr, "[CALLBACK] on_node_basic_info_changed ENTER: type=%d, info=%p\n", static_cast<int>(type), (void*)info);
    fflush(stderr);
    if (!info) return;
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "BusCenter: node info changed: type=%d, name=%s, networkId=%.32s...",
            static_cast<int>(type), info->deviceName, info->networkId);
}

static void on_node_status_changed(NodeStatusType type, NodeStatus* status) {
    fprintf(stderr, "[CALLBACK] on_node_status_changed ENTER: type=%d, status=%p\n", static_cast<int>(type), (void*)status);
    fflush(stderr);
    (void)type;
    (void)status;
}

static INodeStateCb g_node_state_cb = {
    .events = EVENT_NODE_STATE_ONLINE | EVENT_NODE_STATE_OFFLINE | EVENT_NODE_STATE_INFO_CHANGED,
    .onNodeOnline = on_node_online,
    .onNodeOffline = on_node_offline,
    .onNodeBasicInfoChanged = on_node_basic_info_changed,
    .onNodeStatusChanged = on_node_status_changed,
};

static void init_bus_center_if_needed(const char* pkg_name) {
    if (g_bus_center_registered) return;
    if (!pkg_name || !g_RegNodeDeviceStateCb) return;

    // Retry a few times: BusCenter might not be ready right after process start.
    for (int attempt = 1; attempt <= 5; ++attempt) {
        int32_t ret = g_RegNodeDeviceStateCb(pkg_name, &g_node_state_cb);
        if (ret == 0) {
            g_bus_center_registered = true;
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "BusCenter initialized (RegNodeDeviceStateCb OK)");
            return;
        }
        RCUTILS_LOG_WARN_NAMED("rmw_dsoftbus", "RegNodeDeviceStateCb failed: %d (attempt %d/5)", ret, attempt);
        usleep(200000);  // 200ms
    }
}
#endif  // RMW_DSOFTBUS_USE_REAL_SOFTBUS
...
static int OnSessionOpened(int sessionId, int result) {
    fprintf(stderr, "[CALLBACK] OnSessionOpened ENTER: sessionId=%d, result=%d\n", sessionId, result);
    fflush(stderr);
    ...
    if (it != g_state.sessions.end()) {
        ...
        if (g_GetMySessionName) {
            ...
            if (g_GetMySessionName(sessionId, session_name_buf, sizeof(session_name_buf)) == 0) {
                ...
                RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Incoming session %d, name='%s'", sessionId, session_name_buf);
            } else {
                info.session_name = "incoming";
                RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Could not get session name for %d", sessionId);
            }
        } else {
            info.session_name = "incoming";
        }
        ...
        // Infer transport from session name suffix (".kh" indicates KH_TRANS stack)
        info.transport = SessionInfo::Transport::STANDARD;
        if (info.session_name.size() >= 3 &&
            info.session_name.compare(info.session_name.size() - 3, 3, ".kh") == 0) {
            info.transport = SessionInfo::Transport::KH_TRANS;
        }

        g_state.sessions[sessionId] = info;
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Registered incoming session %d", sessionId);
    }
    ...
    } else if (result != 0 && it == g_state.sessions.end()) {
        // Callback arrived before caller could register a pending entry (common for some OpenKhSession impls).
        g_state.early_open_results[sessionId] = result;
    }
...
// ============================================================================
// KH_TRANS Fragment Reassembly (for payloads > 984 bytes)
// ============================================================================
...
static std::unordered_map<int, FragmentReassemblyState> g_fragment_states;
static std::mutex g_fragment_mutex;

static void OnSessionClosed(int sessionId) {
    fprintf(stderr, "[CALLBACK] OnSessionClosed ENTER: sessionId=%d\n", sessionId);
    fflush(stderr);
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "OnSessionClosed: sessionId=%d", sessionId);

    {
        ...
        g_state.early_open_results.erase(sessionId);
    }

    {
        std::lock_guard<std::mutex> frag_lock(g_fragment_mutex);
        g_fragment_states.erase(sessionId);
    }
}
...
static void OnBytesReceived(int sessionId, const void* data, unsigned int dataLen) {
    fprintf(stderr, "[CALLBACK] OnBytesReceived ENTER: sessionId=%d, len=%u\n", sessionId, dataLen);
    fflush(stderr);
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", ">>> OnBytesReceived ENTER: sessionId=%d, len=%u", sessionId, dataLen);

    if (!data || dataLen == 0) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "<<< OnBytesReceived EXIT (null data)");
        return;
    }

    // Reassemble KH_TRANS fragments first (if present).
    std::vector<uint8_t> reassembled;
    const uint8_t* actual_bytes = static_cast<const uint8_t*>(data);
    unsigned int actual_len = dataLen;

#ifdef RMW_DSOFTBUS_USE_REAL_SOFTBUS
    if (actual_len >= FRAGMENT_HEADER_SIZE) {
        FragmentHeader hdr;
        memcpy(&hdr, actual_bytes, FRAGMENT_HEADER_SIZE);

        if (hdr.magic == FRAGMENT_MAGIC &&
            hdr.total_frags > 0 &&
            hdr.frag_len > 0 &&
            actual_len >= FRAGMENT_HEADER_SIZE + hdr.frag_len) {

            const uint8_t* frag_data = actual_bytes + FRAGMENT_HEADER_SIZE;
            const uint16_t crc = kh_pack_crc(reinterpret_cast<const unsigned char*>(frag_data), hdr.frag_len);
            if (crc != hdr.crc16) {
                RCUTILS_LOG_WARN_NAMED("rmw_dsoftbus", "Fragment CRC mismatch: session=%d, msg_id=%u, idx=%u",
                        sessionId, hdr.msg_id, hdr.frag_index);
                return;
            }
...

            {
                ...
                auto& state = g_fragment_states[sessionId];

                const bool is_new_msg = (state.msg_id != hdr.msg_id) ||
                                        (state.total_frags != hdr.total_frags) ||
                                        (state.total_len != hdr.total_len);
                const bool is_timeout = (state.last_update_ns != 0) &&
                                        (now_ns - state.last_update_ns > 5ULL * 1000000000ULL);

                if (state.total_frags == 0 || is_new_msg || is_timeout) {
                    state = FragmentReassemblyState();
                    state.msg_id = hdr.msg_id;
                    state.total_frags = hdr.total_frags;
                    state.total_len = hdr.total_len;
                    state.last_update_ns = now_ns;
                    state.frags.resize(hdr.total_frags);
                    state.received.assign(hdr.total_frags, false);
                }

                if (hdr.frag_index >= state.total_frags) {
                    RCUTILS_LOG_WARN_NAMED("rmw_dsoftbus", "Invalid fragment index: %u/%u",
                            hdr.frag_index, state.total_frags);
                    return;
                }

                if (!state.received[hdr.frag_index]) {
                    state.frags[hdr.frag_index].assign(frag_data, frag_data + hdr.frag_len);
                    state.received[hdr.frag_index] = true;
                    state.received_count++;
                    state.last_update_ns = now_ns;
                }

                if (state.received_count == state.total_frags) {
                    reassembled.resize(state.total_len);
                    size_t offset = 0;
                    for (uint16_t i = 0; i < state.total_frags; ++i) {
                        const auto& frag = state.frags[i];
                        memcpy(reassembled.data() + offset, frag.data(), frag.size());
                        offset += frag.size();
                    }
                    g_fragment_states.erase(sessionId);
                    complete = true;
            ...
            actual_bytes = reassembled.data();
            actual_len = static_cast<unsigned int>(reassembled.size());
        }
    }
#endif  // RMW_DSOFTBUS_USE_REAL_SOFTBUS

    // Strip KhDataHead only when the payload is NOT already a valid RMW message header.
    const void* actual_data = actual_bytes;
    if (actual_len < sizeof(MessageHeader) ||
        !reinterpret_cast<const MessageHeader*>(actual_data)->is_valid()) {

        if (actual_len > sizeof(KhDataHead) + 1) {
            const KhDataHead* head = reinterpret_cast<const KhDataHead*>(actual_bytes);
            uint32_t expected_payload = actual_len - sizeof(KhDataHead) - 1;
            if (head->len == expected_payload || head->len == expected_payload + 1) {
                actual_data = actual_bytes + sizeof(KhDataHead);
                actual_len = head->len;
                RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "KhDataHead stripped: idx=%u, payload=%u",
    ...
    if (callback_func) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Calling raw callback...");
        callback_func(sessionId, actual_data, actual_len, callback_user_data);
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Raw callback returned");
    }

    // Queue the raw message for polling
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Queueing message (len=%u)...", actual_len);
    {
        std::lock_guard<std::mutex> lock(g_state.mutex);
    ...
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Message queued, checking header...");
    ...
    if (actual_len < sizeof(MessageHeader)) {
        ...
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "<<< OnBytesReceived EXIT (too small for header)");
        return;
    }
    ...
    if (!header->is_valid()) {
        ...
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "<<< OnBytesReceived EXIT (invalid header)");
        return;
    }

    // Extract payload (PUBLISH uses MessageHeader, REQUEST/RESPONSE use ServiceHeader)
    const uint8_t* payload = nullptr;
    size_t payload_len = 0;
    const ServiceHeader* srv_header = nullptr;

    if (header->msg_type == MessageType::REQUEST || header->msg_type == MessageType::RESPONSE) {
        if (actual_len < sizeof(ServiceHeader)) {
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "<<< OnBytesReceived EXIT (service header too small)");
            return;
        }
        srv_header = static_cast<const ServiceHeader*>(actual_data);
        payload = static_cast<const uint8_t*>(actual_data) + sizeof(ServiceHeader);
        payload_len = actual_len - sizeof(ServiceHeader);
        if (payload_len != srv_header->base.data_length) {
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "<<< OnBytesReceived EXIT (service length mismatch)");
            return;
        }
    } else {
        payload = static_cast<const uint8_t*>(actual_data) + sizeof(MessageHeader);
        payload_len = actual_len - sizeof(MessageHeader);
        if (payload_len != header->data_length) {
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "<<< OnBytesReceived EXIT (length mismatch)");
            return;
        }
    }
    ...
    if (session_it == g_state.sessions.end()) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "<<< OnBytesReceived EXIT (session not found)");
        return;
    }
    ...
    switch (header->msg_type) {
        ...
        case MessageType::REQUEST: {
            // Service request (srv_header/payload already point to ServiceHeader layout)
    ...
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "<<< OnBytesReceived EXIT (normal end)");
}
...
bool dsoftbus_initialize(const char* pkg_name) {
    ...
    install_debug_signal_handler();

    fprintf(stderr, "[DEBUG][dsoftbus_initialize] called: pkg_name=%s, already_initialized=%d\n",
            pkg_name ? pkg_name : "NULL", g_state.initialized ? 1 : 0);
    fflush(stderr);
    ...
    if (!init_native_token()) {
        RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "Warning: Failed to initialize NativeToken, dsoftbus may not work");
    ...
    // Initialize Bus Center (LNN) early for device discovery APIs (GetAllNodeDeviceInfo, etc.)
    init_bus_center_if_needed(pkg_name);
    ...
    if (g_InitSoftBus) {
        ...
        if (ret != 0) {
            RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "ERROR: InitSoftBus failed with ret=%d", ret);
            RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "       This means IPC connection to softbus_server failed!");
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "       CreateSessionServer will not work properly.");
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "       Possible causes:");
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "         - samgr (System Ability Manager) not accessible");
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "         - Process lacks IPC context (not HAP/system service)");
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "         - softbus_server not running");
            // Continue anyway to see if anything still works
        } else {
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "InitSoftBus succeeded - IPC connection established");
        }
    } else {
        RCUTILS_LOG_WARN_NAMED("rmw_dsoftbus", "WARNING: InitSoftBus symbol not found, IPC may not work!");
    }

    // Retry Bus Center registration after InitSoftBus (some builds require IPC ready first)
    init_bus_center_if_needed(pkg_name);
    ...
    if (use_tc_env && (strcmp(use_tc_env, "0") == 0 || strcmp(use_tc_env, "false") == 0)) {
        ...
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "TransportController disabled by environment variable");
    }

    if (g_use_transport_controller) {
        if (!rmw_dsoftbus::TransportController::instance().initialize(pkg_name)) {
            RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "WARNING: TransportController init failed, falling back to legacy mode");
            g_use_transport_controller = false;
        } else {
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "TransportController enabled (auto-select backend based on packet size)");
        }
    }
#endif
...
void dsoftbus_shutdown() {
    ...
    if (g_use_transport_controller) {
        ...
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "TransportController shutdown complete");
    }
#endif
...
#ifdef RMW_DSOFTBUS_USE_REAL_SOFTBUS
        if (pair.second.transport == SessionInfo::Transport::KH_TRANS) {
            if (g_RemoveKhSessionServer) {
                g_RemoveKhSessionServer(g_state.package_name.c_str(), pair.first.c_str());
            }
        } else {
            if (g_RemoveSessionServer) {
                g_RemoveSessionServer(g_state.package_name.c_str(), pair.first.c_str());
            }
        }
#endif
    }
...
#ifdef RMW_DSOFTBUS_USE_REAL_SOFTBUS
    if (g_bus_center_registered && g_UnregNodeDeviceStateCb) {
        int32_t ret = g_UnregNodeDeviceStateCb(&g_node_state_cb);
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "BusCenter unregistered (ret=%d)", ret);
        g_bus_center_registered = false;
    }
#endif
...
int create_session_server(const char* session_name) {
    ...
    if (!g_state.initialized || !session_name) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "create_session_server: not initialized or null name");
        return -1;
    }
...
#ifdef RMW_DSOFTBUS_USE_REAL_SOFTBUS
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "create_session_server: pkgName='%s', sessionName='%s'",
    ...
    if (!g_CreateSessionServer) {
        RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "CreateSessionServer symbol not available");
        return -1;
    }

    int ret = g_CreateSessionServer(g_state.package_name.c_str(), session_name, &g_sessionListener);
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "CreateSessionServer returned: %d", ret);

    // Handle "session name already exists" error (-426442715)
    // This can happen when another process (subscriber/publisher) already created the server.
    // DO NOT remove it - just treat as success and reuse the existing server.
    if (ret == -426442715) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus",
            "Session server '%s' already exists (possibly from another process), reusing it",
            session_name);
        // Return a new server ID and track it locally
        int server_id = g_state.next_server_id++;
        g_state.session_servers[session_name] = SessionServerInfo{server_id, SessionInfo::Transport::STANDARD};
        return server_id;
    }

    if (ret != 0) {
        RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "CreateSessionServer failed: %d", ret);
        return -1;
    }
#endif
    int server_id = g_state.next_server_id++;
...
int create_kh_session_server(const char* session_name) {
    std::lock_guard<std::mutex> lock(g_state.mutex);
...
#ifdef RMW_DSOFTBUS_USE_REAL_SOFTBUS
    if (!g_CreateKhSessionServer) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "CreateKhSessionServer symbol not available");
        return -1;
    }

    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "create_kh_session_server: pkgName='%s', sessionName='%s'",
            g_state.package_name.c_str(), session_name);

    int ret = g_CreateKhSessionServer(g_state.package_name.c_str(), session_name, &g_sessionListener);
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "CreateKhSessionServer returned: %d", ret);

    // Handle "session name already exists" error (-426442715)
    if (ret == -426442715 && g_RemoveKhSessionServer) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "KH session server exists, removing and retrying...");
        g_RemoveKhSessionServer(g_state.package_name.c_str(), session_name);
        usleep(100000);
        ret = g_CreateKhSessionServer(g_state.package_name.c_str(), session_name, &g_sessionListener);
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "CreateKhSessionServer retry returned: %d", ret);
    }

    if (ret != 0) {
        RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "CreateKhSessionServer failed: %d", ret);
        return -1;
    }
#endif

    int server_id = g_state.next_server_id++;
    g_state.session_servers[session_name] = SessionServerInfo{server_id, SessionInfo::Transport::KH_TRANS};
    return server_id;
}
...
#ifdef RMW_DSOFTBUS_USE_REAL_SOFTBUS
    if (it->second.transport == SessionInfo::Transport::KH_TRANS) {
        if (g_RemoveKhSessionServer) {
            g_RemoveKhSessionServer(g_state.package_name.c_str(), session_name);
        }
    } else {
...
int open_session_to_peer(const char* peer_network_id, const char* my_session_name,
    ...
    if (!g_state.initialized || !peer_network_id || !my_session_name) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "open_session_to_peer: invalid params");
        return -1;
    }
    ...
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Opening session to peer: networkId=%.32s...", peer_network_id);
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "  mySession=%s, peerSession=%s", my_session_name, peer_session_name);
    ...
    if (!g_OpenSession) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "OpenSession symbol not available");
        return -1;
    }

    int session_id = g_OpenSession(my_session_name, peer_session_name,
                                   peer_network_id, "", &attr);
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "OpenSession returned: %d", session_id);

    if (session_id <= 0) {
        RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "OpenSession failed: %d", session_id);
        return session_id;
    }
    ...
    // Register pending session and wait for callback
    {
        ...
        if (existing_it != g_state.sessions.end() && existing_it->second.is_connected) {
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Session %d already connected (fast callback)", session_id);
            return session_id;
        }
        ...
        if (!wait_result) {
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Session open timeout (waited %d ms)", timeout_ms);
        ...
        if (pending.result != 0) {
            RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "Session open failed: result=%d", pending.result);
            g_state.sessions.erase(session_id);
            return -1;
        }
    ...
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Session %d connected successfully!", session_id);
    return session_id;
#else
...
int open_kh_session_to_peer(const char* peer_network_id, const char* my_session_name,
                            const char* peer_session_name, int timeout_ms) {
    if (!g_state.initialized || !peer_network_id || !my_session_name) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "open_kh_session_to_peer: invalid params");
        return -1;
    }
    ...
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Opening KH session to peer: networkId=%.32s...", peer_network_id);
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "  mySession=%s, peerSession=%s", my_session_name, peer_session_name);

#ifdef RMW_DSOFTBUS_USE_REAL_SOFTBUS
    if (!g_OpenKhSession) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "OpenKhSession symbol not available");
        return -1;
    }
    ...
    // OpenKhSession signature differs from OpenSession: (my, peer, networkId, groupId, sessionType, khParam)
    // Use TYPE_BYTES and default KH_TRANS parameters.
    char network_id[65] = {0};
    std::strncpy(network_id, peer_network_id, sizeof(network_id) - 1);
    char group_id[1] = {0};  // empty groupId for auto-auth

    KhTransParam kh_param;
    std::memset(&kh_param, 0, sizeof(kh_param));
    kh_param.khTransType = KH_TRANS_NORMAL;
    kh_param.expectDelayType = KH_NO_LIMIT_LANE_DELAY;
    kh_param.expectPacketLossType = KH_NO_LIMIT_PACKET_LOSS;

    int32_t session_id = g_OpenKhSession(my_session_name, peer_session_name,
                                        network_id, group_id, TYPE_BYTES, &kh_param);
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "OpenKhSession returned: %d", session_id);

    if (session_id <= 0) {
        RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "OpenKhSession failed: %d", session_id);
        return session_id;
    }

    {
        ...
        // Handle synchronous/early failure callbacks (callback may fire before OpenKhSession returns).
        auto early_it = g_state.early_open_results.find(session_id);
        if (early_it != g_state.early_open_results.end()) {
            int early_result = early_it->second;
            g_state.early_open_results.erase(early_it);
            RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "KH session open failed (early callback): result=%d", early_result);
            if (g_CloseKhSession) {
                g_CloseKhSession(session_id);
            }
            return -1;
        }
        ...
        auto existing_it = g_state.sessions.find(session_id);
        if (existing_it != g_state.sessions.end() && existing_it->second.is_connected) {
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "KH session %d already connected (fast callback)", session_id);
            return session_id;
        }
        ...
        if (!wait_result) {
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "KH session open timeout (waited %d ms)", timeout_ms);
            if (g_CloseKhSession) {
                g_CloseKhSession(session_id);
            }
            g_state.sessions.erase(session_id);
        ...
        if (pending.result != 0) {
            RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "KH session open failed: result=%d", pending.result);
            if (g_CloseKhSession) {
                g_CloseKhSession(session_id);
            }
            g_state.sessions.erase(session_id);
    ...
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "KH session %d connected successfully!", session_id);
    return session_id;
#else
    std::lock_guard<std::mutex> lock(g_state.mutex);
    int stub_session_id = g_state.next_session_id++;
...
int send_bytes(int session_id, const void* data, unsigned int len) {
    ...
    uint32_t msg_index = 0;
    {
        ...
        msg_index = ++it->second.send_index;
    }

#ifdef RMW_DSOFTBUS_USE_REAL_SOFTBUS
    // Prefer full KH_TRANS stack for multi-message reliability.
    // If this session is KH_TRANS, use KhSendBytes (with fragmentation when needed).
    if (transport == SessionInfo::Transport::KH_TRANS && g_KhSendBytes) {
        constexpr size_t k_kh_max_payload = 984;

        if (len <= k_kh_max_payload) {
            int ret = g_KhSendBytes(session_id, data, len);
            if (ret != 0) {
                RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "KhSendBytes FAILED: session=%d, ret=%d", session_id, ret);
            }
            return ret;
        }

        // Fragment over KH_TRANS: [FragmentHeader][data...], each fragment <= 984 bytes.
        static std::atomic<uint32_t> msg_id_counter{1};
        const uint32_t msg_id = msg_id_counter.fetch_add(1, std::memory_order_relaxed);

        const size_t max_frag_data = k_kh_max_payload - FRAGMENT_HEADER_SIZE;
        const size_t total_frags = (len + max_frag_data - 1) / max_frag_data;
        if (total_frags > UINT16_MAX) {
            RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "KH fragmentation too many fragments: %zu", total_frags);
            return -1;
        }
        ...
        for (size_t i = 0; i < total_frags; ++i) {
            const size_t frag_len = std::min(max_frag_data, static_cast<size_t>(len) - offset);

            std::vector<uint8_t> packet(FRAGMENT_HEADER_SIZE + frag_len);
            auto* hdr = reinterpret_cast<FragmentHeader*>(packet.data());
            ...
            hdr->crc16 = kh_pack_crc(reinterpret_cast<const unsigned char*>(src + offset),
                                     static_cast<uint16_t>(frag_len));
            memset(hdr->reserved, 0, sizeof(hdr->reserved));

            memcpy(packet.data() + FRAGMENT_HEADER_SIZE, src + offset, frag_len);

            int ret = g_KhSendBytes(session_id, packet.data(),
                                    static_cast<unsigned int>(packet.size()));
            if (ret != 0) {
                RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "KH frag send failed: session=%d, frag=%zu/%zu, ret=%d",
                        session_id, i + 1, total_frags, ret);
                return ret;
            }
    ...
    // Standard Session API: SendBytes with manual KhDataHead to avoid dedup drops.
    if (g_SendBytes) {
        KhDataHead head;
        ...
        buffer[sizeof(KhDataHead) + len] = 0;

        int ret = g_SendBytes(session_id, buffer.data(), static_cast<unsigned int>(total_len));
        if (ret != 0) {
            RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "SendBytes FAILED: session=%d, ret=%d", session_id, ret);
        }
        return ret;
    }
#endif
...
int get_local_device_info(char* network_id, unsigned int nid_len,
    ...
    if (g_GetLocalNodeDeviceInfo) {
        ...
        if (ret == 0) {
            ...
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Local device: networkId=%s, name=%s",
                    info.networkId, info.deviceName);
            return 0;
        }
        RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "GetLocalNodeDeviceInfo failed: %d", ret);
        return ret;
    }
#endif
    // Stub mode
    ...
    *device_count = 0;
    ...
    if (g_GetAllNodeDeviceInfo) {
        ...
        if (ret == 0 && info != nullptr) {
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Found %d online devices:", count);
            for (int i = 0; i < count; i++) {
                RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "  [%d] networkId=%s, name=%s",
        ...
        RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "GetAllNodeDeviceInfo failed: %d", ret);
        return ret;
    }
#endif
...
std::string encode_topic_for_session(const char* topic_name) {
    ...
    std::string result = g_state.package_name.empty() ? RMW_DSOFTBUS_PACKAGE_NAME : g_state.package_name;
...
int create_topic_subscription(const char* topic_name, const char* type_name, void* subscription_impl) {
    ...
    std::string kh_encoded = encoded + ".kh";
    ...
    if (server_id < 0) {
        RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "Failed to create session server for topic %s", topic_name);
        return -1;
    }

    // Best-effort: Create KH_TRANS session server for better multi-message delivery
    int kh_server_id = create_kh_session_server(kh_encoded.c_str());
    if (kh_server_id >= 0) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Created KH topic session server: %s", kh_encoded.c_str());
    }
    ...
    if (kh_server_id >= 0) {
        register_subscription(kh_encoded.c_str(), subscription_impl);
    }

    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Created topic subscription: %s -> session '%s'",
...
void destroy_topic_subscription(const char* topic_name) {
    ...
    remove_session_server(kh_encoded.c_str());

    // Unregister from routing
    unregister_subscription(encoded.c_str());
    unregister_subscription(kh_encoded.c_str());

    g_topic_subscriptions.erase(it);

    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Destroyed topic subscription: %s", topic_name);
}
...
int publish_to_topic(const char* topic_name, const void* data, unsigned int len,
    ...
    // ========== LOCAL INTRA-PROCESS DELIVERY ==========
    // Check if there are local subscriptions for this topic and deliver directly
    int local_delivery_count = 0;
    {
        std::lock_guard<std::mutex> lock(g_topic_mutex);
        auto it = g_topic_subscriptions.find(encoded);
        if (it != g_topic_subscriptions.end() && it->second.subscription_impl) {
            // For RMW-level subscriptions (with valid SubscriptionImpl)
            // we use notify_subscription. But first check if this looks like
            // a valid SubscriptionImpl by checking subscription_impl->is_rmw_subscription
            // which we set to true for RMW-created subscriptions.
            //
            // For test-level subscriptions (pubsub_test.cpp), the raw_callback is used.

            local_delivery_count++;
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus",
                "publish_to_topic: found local subscription on %s", topic_name);
        }
    }

    // Also call raw message callback if set (for tests and simple clients)
    // This is safer than casting to SubscriptionImpl which may not be valid
    {
        std::lock_guard<std::mutex> lock(g_state.mutex);
        if (g_state.raw_callback) {
            // Call with session_id = -1 to indicate local delivery
            g_state.raw_callback(-1, buffer.data(), buffer.size(), g_state.raw_callback_user_data);
            local_delivery_count++;
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus",
                "publish_to_topic: delivered %zu bytes via raw_callback (local)",
                buffer.size());
        }
    }
    ...
    if (g_use_transport_controller &&
        ...
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "publish_to_topic: SOCKET_ONLY mode - using TransportController");
        ...
        if (get_all_online_devices(&device_list, &device_count) == 0 && device_count > 0) {
            ...
            for (int i = 0; i < device_count; i++) {
                ...
                RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "SOCKET_ONLY: Attempting to connect to peer %s (%.32s...)",
                ...
                if (conn_id > 0) {
                    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "SOCKET_ONLY: Connected via TransportController, conn_id=%d", conn_id);
                    ...
                    if (ret == 0) {
                        success_count++;
                        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "SOCKET_ONLY: Sent %zu bytes to peer %s",
                                buffer.size(), nodes[i].deviceName);
                    } else {
                        RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "SOCKET_ONLY: Send failed: %d", ret);
                    }

                    // Note: Not disconnecting here - keep connection for future messages
                } else {
                    RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "SOCKET_ONLY: Connection failed: %d", conn_id);
                }
            }
            free_device_list(device_list);
        }

        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "SOCKET_ONLY: Sent to %d remote peers, %d local",
            success_count, local_delivery_count);
        return success_count + local_delivery_count;
    }
    ...
    // Ensure we have a session server for this topic (required by dsoftbus for OpenSession)
    {
        ...
        if (g_publisher_session_servers.find(encoded) == g_publisher_session_servers.end()) {
            ...
            if (server_id >= 0) {
                ...
                RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Publisher created session server for topic: %s", encoded.c_str());
            }
        }

        // Best-effort: Create KH_TRANS publisher session server (required by OpenKhSession)
        if (g_publisher_session_servers.find(kh_encoded) == g_publisher_session_servers.end()) {
            int kh_server_id = create_kh_session_server(kh_encoded.c_str());
            if (kh_server_id >= 0) {
                g_publisher_session_servers[kh_encoded] = kh_server_id;
                RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Publisher created KH session server for topic: %s", kh_encoded.c_str());
            }
        }
    }

    // Prefer KH_TRANS sessions first (better multi-message reliability), then fall back to standard sessions.
    std::vector<int> kh_sessions = get_sessions_for_topic(kh_encoded.c_str());
    std::vector<int> sessions = get_sessions_for_topic(encoded.c_str());
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "publish_to_topic: found %zu KH sessions, %zu standard sessions for %s",
            kh_sessions.size(), sessions.size(), encoded.c_str());

    std::unordered_set<std::string> sent_peers;
    int success_count = 0;
    auto mark_sent_peer = [&](int sid) {
        std::lock_guard<std::mutex> lock(g_state.mutex);
        auto it = g_state.sessions.find(sid);
        if (it != g_state.sessions.end() && !it->second.peer_device_id.empty()) {
            sent_peers.insert(it->second.peer_device_id);
        }
    };

    for (int sid : kh_sessions) {
    ...
    for (int sid : sessions) {
        {
            ...
            auto it = g_state.sessions.find(sid);
            if (it != g_state.sessions.end() && !it->second.peer_device_id.empty() &&
                sent_peers.find(it->second.peer_device_id) != sent_peers.end()) {
                continue;  // Prefer KH session if both exist for same peer
            }
        }
    ...
    // Discover peers and connect to those we haven't sent to yet.
    void* device_list = nullptr;
    int device_count = 0;
    if (get_all_online_devices(&device_list, &device_count) == 0 && device_count > 0) {
        char local_nid[128] = {0};
        ...
        for (int i = 0; i < device_count; i++) {
            ...
            if (sent_peers.find(nodes[i].networkId) != sent_peers.end()) {
                continue;
            }

            int session_id = -1;

            // Try KH_TRANS first (requires both ends to have ".kh" server)
            session_id = open_kh_session_to_peer(nodes[i].networkId,
                                                 kh_encoded.c_str(),
                                                 kh_encoded.c_str(),
                                                 3000);
            if (session_id > 0) {
                ...
                if (ret == 0) {
                    success_count++;
                    sent_peers.insert(nodes[i].networkId);
                }
                continue;
            }

            // Fallback: standard Session API
            session_id = open_session_to_peer(nodes[i].networkId,
                                              encoded.c_str(),
                                              encoded.c_str(),
                                              3000);
            if (session_id > 0) {
    ...
    // ========== LOCAL INTER-PROCESS DELIVERY ==========
    // Note: dsoftbus doesn't support OpenSession to the same device (error -426115063).
    // Same-device inter-process communication is a known limitation.
    // Workarounds:
    // 1. Use 'both' mode in pubsub_test for intra-process testing
    // 2. Use two devices for true inter-process pub/sub testing
    // 3. Future: implement Unix domain socket or shared memory IPC
    if (success_count == 0 && local_delivery_count == 0) {
        RCUTILS_LOG_WARN_NAMED("rmw_dsoftbus",
            "publish_to_topic: no remote peers and no local subscribers found for %s. "
            "Note: same-device inter-process communication requires separate devices or "
            "use 'both' mode for single-device testing.", topic_name);
    }

    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "publish_to_topic: sent to %d remote peers, %d local",
        success_count, local_delivery_count);
    return success_count + local_delivery_count;
}
...
int create_service_server(const char* service_name, const char* type_name, void* service_impl) {
    ...
    int server_id = create_session_server(encoded.c_str());
    if (server_id < 0) {
        RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "Failed to create session server for service %s", service_name);
        return -1;
    }
    ...
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Created service server: %s -> session '%s'",
...
void destroy_service_server(const char* service_name) {
    ...
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Destroyed service server: %s", service_name);
}
...
int send_service_response(const char* service_name, int client_session_id,
    ...
    if (ret == 0) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Sent service response: request_id=%ld, session=%d, len=%u",
...
int create_service_client(const char* service_name, const char* type_name, void* client_impl) {
    ...
    if (client_session_servers.find(encoded) == client_session_servers.end()) {
        ...
        if (server_id >= 0) {
            ...
            RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Client created session server for service: %s", encoded.c_str());
        }
    }
    ...
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Created service client: %s -> session '%s'",
            service_name, encoded.c_str());
...
void destroy_service_client(const char* service_name) {
    ...
    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Destroyed service client: %s", service_name);
}
...
int send_service_request(const char* service_name, int64_t request_id,
        ...
        if (get_all_online_devices(&device_list, &device_count) == 0 && device_count > 0) {
            ...
            for (int i = 0; i < device_count; i++) {
                ...
                if (sid > 0) {
                    ...
                    RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Connected to service %s on peer %s, session=%d",
    ...
    if (session_id < 0) {
        RCUTILS_LOG_ERROR_NAMED("rmw_dsoftbus", "Failed to find service: %s", service_name);
        return -1;
    }
    ...
    int ret = send_bytes(session_id, buffer.data(), buffer.size());
    if (ret == 0) {
        RCUTILS_LOG_INFO_NAMED("rmw_dsoftbus", "Sent service request: request_id=%ld, session=%d, len=%u",
