...
// sensor_msgs/msg/NavSatStatus functions implementation
...
sensor_msgs__msg__NavSatStatus * sensor_msgs__msg__NavSatStatus__create(void)
{
  ...
  sensor_msgs__msg__NavSatStatus * msg =
    (sensor_msgs__msg__NavSatStatus *)allocator.allocate(
      sizeof(sensor_msgs__msg__NavSatStatus), allocator.state);
  if (!msg) { return NULL; }
  if (!sensor_msgs__msg__NavSatStatus__init(msg)) {
  ...
  sensor_msgs__msg__NavSatStatus__Sequence * array, size_t size)
{
  ...
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (size) {
    array->data = (sensor_msgs__msg__NavSatStatus *)allocator.zero_allocate(
      size, sizeof(sensor_msgs__msg__NavSatStatus), allocator.state);
    if (!array->data) { return false; }
    for (size_t i = 0; i < size; ++i) {
      if (!sensor_msgs__msg__NavSatStatus__init(&array->data[i])) {
        for (size_t j = 0; j < i; ++j) {
          sensor_msgs__msg__NavSatStatus__fini(&array->data[j]);
        }
