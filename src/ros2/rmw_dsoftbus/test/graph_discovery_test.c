...
// Note: Token functions are loaded via dlopen to avoid static lib dependency
// Use com.huawei.ros2_rmw_dsoftbus which was successful in Phase 2
#define PKG_NAME "com.huawei.ros2_rmw_dsoftbus"
#define SESSION_NAME "com.huawei.ros2_rmw_dsoftbus.graph"
...
// ============ Set permissions (dlopen-based) ============

static void SetPermissions(void)
{
    printf("[INFO] Setting permissions (dlopen)...\n");

    // Try multiple library paths for token functions
    const char *tokenLibPaths[] = {
        "/system/lib64/libnativetoken.z.so",
        "/system/lib64/platformsdk/libnativetoken.z.so",
        "/system/lib64/libnativetoken_json.z.so",
        "/system/lib64/platformsdk/libnativetoken_json.z.so",
        NULL
    };

    void *tokenHandle = NULL;
    for (int i = 0; tokenLibPaths[i] != NULL; i++) {
        tokenHandle = dlopen(tokenLibPaths[i], RTLD_NOW);
        if (tokenHandle) {
            printf("[INFO] Loaded token library: %s\n", tokenLibPaths[i]);
            break;
        }
    }

    if (!tokenHandle) {
        printf("[WARN] Cannot load any token library: %s\n", dlerror());
        printf("[INFO] Continuing without explicit permission setting...\n");
        return;
    }

    GetAccessTokenId_t fn_GetAccessTokenId = (GetAccessTokenId_t)dlsym(tokenHandle, "GetAccessTokenId");
    SetSelfTokenID_t fn_SetSelfTokenID = (SetSelfTokenID_t)dlsym(tokenHandle, "SetSelfTokenID");

    if (!fn_GetAccessTokenId || !fn_SetSelfTokenID) {
        printf("[WARN] Token functions not found: %s\n", dlerror());
        printf("[INFO] Continuing without explicit permission setting...\n");
        dlclose(tokenHandle);
        return;
    }
...
{
    printf("==============================================\n");
    printf("Graph Discovery & Session Test\n");
    printf("PKG_NAME: %s\n", PKG_NAME);
    printf("SESSION_NAME: %s\n", SESSION_NAME);
    printf("==============================================\n\n");
