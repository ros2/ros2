...
// sensor_msgs/msg/TimeReference functions implementation
...
bool sensor_msgs__msg__TimeReference__init(sensor_msgs__msg__TimeReference * msg)
{
  ...
  if (!builtin_interfaces__msg__Time__init(&msg->time_ref)) { return false; }
  if (!rosidl_runtime_c__String__init(&msg->source)) { return false; }
  return true;
}
...
sensor_msgs__msg__TimeReference * sensor_msgs__msg__TimeReference__create(void)
{
  ...
  sensor_msgs__msg__TimeReference * msg =
    (sensor_msgs__msg__TimeReference *)allocator.allocate(
      sizeof(sensor_msgs__msg__TimeReference), allocator.state);
  if (!msg) { return NULL; }
  if (!sensor_msgs__msg__TimeReference__init(msg)) {
  ...
  sensor_msgs__msg__TimeReference__Sequence * array, size_t size)
{
  ...
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (size) {
    array->data = (sensor_msgs__msg__TimeReference *)allocator.zero_allocate(
      size, sizeof(sensor_msgs__msg__TimeReference), allocator.state);
    if (!array->data) { return false; }
    for (size_t i = 0; i < size; ++i) {
      if (!sensor_msgs__msg__TimeReference__init(&array->data[i])) {
        for (size_t j = 0; j < i; ++j) {
          sensor_msgs__msg__TimeReference__fini(&array->data[j]);
        }
