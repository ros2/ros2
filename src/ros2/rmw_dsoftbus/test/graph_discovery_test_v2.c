/**
 * Graph Discovery & Session Cross-Device Test V2
 *
 * Uses dlopen for ALL external functions to avoid static linking issues
 * Both permissions: DISTRIBUTED_DATASYNC + DISTRIBUTED_SOFTBUS_CENTER
 */
...
// ============ NativeTokenInfoParams definition ============
...
static void *g_tokenSetprocHandle = NULL;
...
// ============ Load libraries via dlopen ============

static int LoadTokenLibraries(void)
{
    printf("[INFO] Loading token libraries...\n");

    // Try to load accesstoken_sdk library (contains GetAccessTokenId)
    const char *accessTokenPaths[] = {
        ACCESSTOKEN_SDK_LIB_PATH,
        "/system/lib64/platformsdk/libaccesstoken_sdk.z.so",
        "/system/lib64/libaccesstoken_sdk.z.so",
        NULL
    };

    for (int i = 0; accessTokenPaths[i] != NULL; i++) {
        printf("[INFO] Trying: %s\n", accessTokenPaths[i]);
        g_tokenHandle = dlopen(accessTokenPaths[i], RTLD_NOW | RTLD_GLOBAL);
        if (g_tokenHandle) {
    ...
    if (g_tokenHandle) {
        ...
        if (!fn_GetAccessTokenId) {
            printf("[WARN] GetAccessTokenId not found in accesstoken_sdk\n");
        } else {
            printf("[INFO] GetAccessTokenId found: %p\n", (void*)fn_GetAccessTokenId);
        }

        // Also try to find SetSelfTokenID in the same library
        fn_SetSelfTokenID = (SetSelfTokenID_t)dlsym(g_tokenHandle, "SetSelfTokenID");
        if (fn_SetSelfTokenID) {
            printf("[INFO] SetSelfTokenID found: %p\n", (void*)fn_SetSelfTokenID);
        }
    } else {
        printf("[WARN] Could not load accesstoken_sdk library\n");
    }

    // If SetSelfTokenID not found, try tokenid_sdk
    if (!fn_SetSelfTokenID) {
        const char *tokenIdPaths[] = {
            TOKENID_SDK_LIB_PATH,
            "/system/lib64/chipset-pub-sdk/libtokenid_sdk.z.so",
            "/system/lib64/libtokenid_sdk.z.so",
            NULL
        };

        for (int i = 0; tokenIdPaths[i] != NULL; i++) {
            printf("[INFO] Trying tokenid: %s\n", tokenIdPaths[i]);
            g_tokenSetprocHandle = dlopen(tokenIdPaths[i], RTLD_NOW | RTLD_GLOBAL);
            if (g_tokenSetprocHandle) {
                printf("[INFO] Loaded tokenid: %s\n", tokenIdPaths[i]);
                break;
            }
            printf("[WARN] Failed tokenid: %s\n", dlerror());
        }

        if (g_tokenSetprocHandle) {
            fn_SetSelfTokenID = (SetSelfTokenID_t)dlsym(g_tokenSetprocHandle, "SetSelfTokenID");
            if (!fn_SetSelfTokenID) {
                printf("[WARN] SetSelfTokenID not found in tokenid_sdk\n");
            } else {
                printf("[INFO] SetSelfTokenID found in tokenid_sdk: %p\n", (void*)fn_SetSelfTokenID);
            }
        }
    }

    if (fn_GetAccessTokenId && fn_SetSelfTokenID) {
        printf("[INFO] Token libraries loaded successfully\n");
        return 0;
    }

    printf("[WARN] Token libraries not fully loaded, continuing without permissions\n");
    return -1;
}
...
static void SetPermissions(void)
{
    ...
    if (!fn_GetAccessTokenId || !fn_SetSelfTokenID) {
        printf("[WARN] Token functions not available, skipping permission setup\n");
        return;
    }
    ...
    printf("[INFO] GetAccessTokenId: tokenId=0x%llx\n", (unsigned long long)tokenId);
...
static int RunAsClient(const char *peerNetworkId)
{
    ...
    const char *msg = "Hello from rmw_dsoftbus client!";
...
{
    printf("==============================================\n");
    printf("Graph Discovery & Session Test V2\n");
    printf("PKG_NAME: %s\n", PKG_NAME);
    printf("SESSION_NAME: %s\n", SESSION_NAME);
    printf("==============================================\n\n");

    // Step 1: Load token libraries (optional, may fail)
    LoadTokenLibraries();

    // Step 2: Set permissions (if token libraries loaded)
    SetPermissions();

    // Step 3: Load dsoftbus library
    if (LoadSoftbusLibrary() != 0) {
        printf("[FATAL] Cannot load dsoftbus library!\n");
        return 1;
    }

    // Step 4: Get local node info
    ...
    if (!serverMode && peerNetworkId == NULL && nodes != NULL && nodeCount > 0) {
        ...
        if (fn_GetLocalNodeDeviceInfo && fn_GetLocalNodeDeviceInfo(PKG_NAME, &localNodeInfo) == 0) {
            for (int i = 0; i < nodeCount; i++) {
                if (strcmp(nodes[i].networkId, localNodeInfo.networkId) != 0) {
                    peerNetworkId = nodes[i].networkId;
                    printf("[INFO] Auto-selected peer: %s (%s)\n",
            ...
            printf("[INFO] Using first device as peer: %s\n", peerNetworkId);
        }
    }
    ...
    if (g_tokenHandle) {
        dlclose(g_tokenHandle);
    }
    if (g_tokenSetprocHandle) {
        dlclose(g_tokenSetprocHandle);
    }
